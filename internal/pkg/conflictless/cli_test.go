...
func TestParseCLIFlags(t *testing.T) {
	...
	for _, testCase := range []parseCliFlagsTestCase{
		...
		{
			"generate with v prefix",
			[]string{"generate", "-c", changelog, "--use-v-prefix-in-version-links", "-d", dir, "-b", bump},
			conflictless.FlagCollection{
				Bump:                     &bump,
				ChangeFileFormat:         nil,
				ChangeFileName:           nil,
		...
		{
			"preview with v prefix",
			[]string{"preview", "--changelog", changelog, "-p", "--dir", dir, "--bump", bump},
			conflictless.FlagCollection{
				Bump:                     &bump,
				ChangeFileFormat:         nil,
				ChangeFileName:           nil,
